-- ScopeLogic v1.0 RC5.5 — Internal Matrix Source Reference Standard
-- Adds an explicit Source Type alongside the existing Source Reference field.

alter table public.slr_entries
  add column if not exists source_type text not null default '';

comment on column public.slr_entries.source_type is
  'Source classification for an SLR, e.g. Drawing, Specification, Addendum, Existing Condition, or Scope Omission.';
