...
function takeoffRuleRecentFive() {
  ...
  if (filtering) options.forEach((option) => { option.hidden = false; });
  else options.forEach((option, index) => { option.hidden = index >= 5 && option.value !== select.value; });
}
...
function refreshUniformity() {
  installPanelToggles(); quoteTemplateRecentFive(); slrRecentFive(); takeoffRuleRecentFive(); bindSearchRefresh();
}
