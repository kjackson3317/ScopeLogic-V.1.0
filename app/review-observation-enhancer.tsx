...
function CaptureGroupResolve({masterId}:{masterId:string}){
 ...
 const identity=useCallback(async():Promise<Identity>=>{
  ...
  if(result.error||!result.data?.user)throw new Error('Your ScopeLogic session is not available. Sign in again.');
 ...
 return <section className="sl-review-workflow">
  ...
  {tab==='groups'?<div className="sl-review-evidence">
   ...
   <div className="sl-review-manual-note"><b>Manual SLR workflow</b><span>Evidence Groups are reference material. ScopeLogic will not create a new SLR from a group. Pop this view out and create the SLR yourself in the Internal Review Matrix.</span></div>
...
export default function ReviewObservationEnhancer(){
 ...
 useEffect(()=>{let queued=false;const install=()=>{const match=window.location.pathname.match(/^\/master-projects\/([^/]+)\/deliverables\/?$/);if(!match){setMount(null);return false}setMasterId(match[1]);const title=Array.from(document.querySelectorAll('h2')).find(node=>node.textContent?.trim()==='Review Notes');if(!title){setMount(null);return false}const workspace=title.closest('section');if(!workspace)return false;const head=title.parentElement?.parentElement as HTMLElement|null;const subtitle=title.parentElement?.querySelector('p');if(subtitle)subtitle.textContent='Capture raw evidence, group it by System + Topic, then keep the Evidence Groups open as a reference while you manually create or update SLRs.';
