...
function ensureHost(selector:string,className:string,after?:Element|null){
  ...
  if(host){
    if(after?.parentElement&&host.previousElementSibling!==after)after.after(host);
    return host;
  }
...
function SlrDeliverablesEditor(){
  ...
  const [collapsed,setCollapsed]=useState<Record<'CL'|'VE',boolean>>({CL:false,VE:false});
  ...
  const clItems=linked.filter((item)=>item.deliverable_type==='CL');
  ...
  useEffect(()=>{if(slrId){setCollapsed({CL:false,VE:false});void load(slrId);}},[slrId]);

  const startNew=(type:'CL'|'VE')=>{setCollapsed((current)=>({...current,[type]:false}));setNewDraft({type,system_name:finding?.systems?.[0]||'',title:finding?.scope_item||'',content:'',impact_considerations:'',reference:'',status:type==='VE'?'Identified':'Draft',response:'',response_source:''});};
  const saveNew=async()=>{
