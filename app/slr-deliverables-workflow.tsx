...
function DeliverableEditorRow({item,onSaved,onDeleted}:{item:Action;onSaved:(item:Action)=>void;onDeleted:(id:string)=>void}){
  ...
  const remove=async()=>{if(!confirmDelete){setConfirmDelete(true);return;}const result=await supabase.from('master_project_deliverable_items').delete().eq('id',item.id);if(!result.error)onDeleted(item.id);};
