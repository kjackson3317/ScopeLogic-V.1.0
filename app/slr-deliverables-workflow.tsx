...
function SlrDeliverablesEditor(){
  ...
  useEffect(()=>{
    let queued=false;
    const refresh=()=>{if(queued)return;queued=true;window.requestAnimationFrame(()=>{queued=false;const editor=document.querySelector<HTMLElement>('.matrix-editor-full .slr-child-editor');const id=currentSlrId();if(editor){const nextHost=ensureHost('.matrix-editor-full .sl-slr-deliverables-host','sl-slr-deliverables-host',editor);setHost(nextHost);}else setHost(null);setSlrId((current)=>{if(id&&id!==current){void load(id);return id;}return id||'';});});};
  ...
  const saveNew=async()=>{if(!newDraft||!finding||!masterId||!newDraft.title.trim()||!newDraft.content.trim())return;const sameType=allActions.filter((item)=>item.deliverable_type===newDraft.type);const seq=Math.max(0,...sameType.map((item)=>Number(item.sequence_number)||0))+1;const userResult=await supabase.auth.getUser();const user=userResult.data?.user;const ownerId=String(user?.app_metadata?.scopelogic_workspace_owner_id||user?.id||'');const actualId=String(user?.app_metadata?.scopelogic_actual_user_id||user?.id||ownerId);const payload={owner_id:ownerId,master_project_id:masterId,created_by_user_id:actualId,related_master_finding_id:finding.id,deliverable_type:newDraft.type,sequence_number:seq,display_number:`${newDraft.type}-${pad(seq)}`,system_name:newDraft.system_name,title:newDraft.title.trim(),content:newDraft.content.trim(),impact_considerations:newDraft.impact_considerations.trim(),reference:newDraft.reference.trim(),status:newDraft.status,response:newDraft.response.trim(),response_date:null,response_source:newDraft.response_source.trim(),client_facing:true,sort_order:seq};const result=await supabase.from('master_project_deliverable_items').insert(payload);if(result.error){setMessage(result.error.message||'Could not save deliverable.');return;}setNewDraft(null);setMessage(`${payload.display_number} saved to ${slrId}.`);await load(slrId);};

  if(!host||!slrId)return null;
  return createPortal(<section className="sl-slr-deliverables"><div className="sl-slr-deliverables-heading"><div><span>SLR DELIVERABLES</span><h3>GC Clarifications & VE Opportunities</h3><p>These items are editable only from their originating SLR. The Deliverables menu is read-only preview.</p></div><div><button type="button" className="secondary" onClick={()=>startNew('CL')}>+ GC Clarification</button><button type="button" className="secondary" onClick={()=>startNew('VE')}>+ VE Opportunity</button></div></div>{loading?<div className="sl-preview-state">Loading linked deliverables…</div>:!finding?<div className="sl-preview-state">This SLR is not yet linked to the Master Project deliverables table. Submit/save the SLR first, then reopen it.</div>:<>{message&&<div className="sl-slr-message">{message}</div>}{newDraft&&<article className="sl-slr-deliverable-card new"><div className="sl-slr-deliverable-card-head"><div><b>NEW {newDraft.type}</b><span>{newDraft.type==='CL'?'GC Clarification':'VE Opportunity'}</span></div><button type="button" className="secondary" onClick={()=>setNewDraft(null)}>Cancel</button></div><div className="sl-slr-deliverable-grid"><label><span>System</span><input value={newDraft.system_name} onChange={(e)=>setNewDraft({...newDraft,system_name:e.target.value})}/></label><label><span>Title / Subject</span><input value={newDraft.title} onChange={(e)=>setNewDraft({...newDraft,title:e.target.value})}/></label><label className="wide"><span>{newDraft.type==='VE'?'VE Opportunity':'GC Clarification'}</span><textarea rows={3} value={newDraft.content} onChange={(e)=>setNewDraft({...newDraft,content:e.target.value})}/></label>{newDraft.type==='VE'?<label className="wide"><span>Potential Impact / Considerations</span><textarea rows={2} value={newDraft.impact_considerations} onChange={(e)=>setNewDraft({...newDraft,impact_considerations:e.target.value})}/></label>:<label className="wide"><span>Response / Resolution</span><textarea rows={2} value={newDraft.response} onChange={(e)=>setNewDraft({...newDraft,response:e.target.value})}/></label>}<label className="wide"><span>Document Reference</span><input value={newDraft.reference} onChange={(e)=>setNewDraft({...newDraft,reference:e.target.value})}/></label></div><div className="sl-slr-deliverable-actions"><button type="button" className="primary" disabled={!newDraft.title.trim()||!newDraft.content.trim()} onClick={saveNew}>Save to SLR</button></div></article>}{linked.filter((item)=>item.deliverable_type==='CL').length>0&&<div className="sl-slr-subhead">GC Clarifications</div>}{linked.filter((item)=>item.deliverable_type==='CL').map((item)=><DeliverableEditorRow key={item.id} item={item} onSaved={(next)=>setAllActions((current)=>current.map((x)=>x.id===next.id?next:x))} onDeleted={(id)=>setAllActions((current)=>current.filter((x)=>x.id!==id))}/>)}{linked.filter((item)=>item.deliverable_type==='VE').length>0&&<div className="sl-slr-subhead">VE Opportunities</div>}{linked.filter((item)=>item.deliverable_type==='VE').map((item)=><DeliverableEditorRow key={item.id} item={item} onSaved={(next)=>setAllActions((current)=>current.map((x)=>x.id===next.id?next:x))} onDeleted={(id)=>setAllActions((current)=>current.filter((x)=>x.id!==id))}/>)}{!linked.length&&!newDraft&&<div className="sl-preview-state">No GC Clarification or VE Opportunity is linked to this SLR yet.</div>}</>}</section>,host);
}
