...
type QuoteLine = { id: string; partId: string; manufacturer: string; partNumber: string; description: string; system: string; bomSection?: string; groupId?: string; breakoutId?: string; alternateId?: string; showOnBom?: boolean; qty: number; unitCost: number; materialMarkup: number; materialMarkupOverride?: number | null; engineeringMinutes: number; installationMinutes: number; programmingMinutes: number; testingMinutes: number; laborMinutes?: Record<string, number>; cableType?: string; cableFeet?: number; adHoc?: boolean; quantitySources?: QuoteLineQuantitySources; takeoffGenerated?: boolean; keepZero?: boolean };
...
type Quote = { id: string; number: string; name: string; status: string; taxRate: number; bondRate: number; shipping: number; shippingMarkup?: number; otherCosts: number; otherCostsMarkup?: number; lines: QuoteLine[]; groups?: QuoteGroup[]; breakouts?: QuoteBreakout[]; alternates?: QuoteAlternate[]; createdAt: string; updatedAt: string; difficultyId?: string; globalMaterialMarkup?: number; laborMarkups?: Record<string, number>; projectManagementHours?: number; travelHours?: Record<string, number>; travel?: TravelCalculator; laborAdjustments?: Record<string, number>; jobMaterialDiscount?: number; perDiemTravel?: number; terms?: string; internalNotes?: string; adminNotes?: string; engineeringNotRequired?: boolean; quoteKind?: 'base' | 'change-order'; quoteYear?: number; rootSequence?: number; changeOrderNumber?: number; revisionNumber?: number; parentQuoteId?: string; locked?: boolean };
...
type WorkspaceBackupFile = {
  ...
  version: '1.0';
  applicationVersion: '1.0.0-rc.5.5.5';
  exportedAt: string;
...
const formatQuoteNumber = (quote: Pick<Quote, 'quoteYear' | 'rootSequence' | 'quoteKind' | 'changeOrderNumber' | 'revisionNumber'>) => {
  const year = quote.quoteYear || new Date().getFullYear();
  const root = String(quote.rootSequence || 1).padStart(5, '0');
  const revision = String(quote.revisionNumber || 0).padStart(2, '0');
  const changeOrder = quote.quoteKind === 'change-order' ? `-CO${String(quote.changeOrderNumber || 1).padStart(2, '0')}` : '';
  return `SL-${year}-${root}${changeOrder}-R${revision}`;
};
const parseQuoteNumber = (value: string) => {
  const match = /^SL-(\d{4})-(\d{5})(?:-CO(\d{2}))?-R(\d{2})$/i.exec(String(value || '').trim());
  if (!match) return null;
  return { quoteYear: Number(match[1]), rootSequence: Number(match[2]), quoteKind: match[3] ? 'change-order' as const : 'base' as const, changeOrderNumber: match[3] ? Number(match[3]) : undefined, revisionNumber: Number(match[4]) };
};
const nextRootSequence = (quotes: Quote[], year = new Date().getFullYear()) => Math.max(0, ...quotes.map((quote) => parseQuoteNumber(quote.number)).filter((item): item is NonNullable<ReturnType<typeof parseQuoteNumber>> => Boolean(item && item.quoteYear === year)).map((item) => item.rootSequence)) + 1;
...
export default function Workspace({ userEmail }: { userEmail: string; userId: string }) {
  ...
  return (
    ...
    <div className="brand"><div className="brand-mark"><img src="/brand/scopelogic-logo-mark.png" alt="ScopeLogic" /></div><div><div className="brand-name-box"><img className="brand-wordmark" src="/brand/scopelogic-wordmark.png" alt="ScopeLogic" /></div><span>v1.0 RC5.5.5</span></div></div>
...
function SystemStatus({ dataMode, syncState, syncError, cloudStatus, retryCloudSync, docsByProject, exportBackup, chooseBackup, exportWorkspace, chooseWorkspace, createRestorePoint, backups, backupLoading, restorePoint }: { dataMode: 'cloud' | 'local-fallback' | 'loading'; syncState: 'loading' | 'synced' | 'saving' | 'error'; syncError: string; cloudStatus: CloudWorkspaceStatus; retryCloudSync: () => void | Promise<void>; docsByProject: Record<string, Doc[]>; exportBackup: () => void | Promise<void>; chooseBackup: () => void; exportWorkspace: () => void; chooseWorkspace: () => void; createRestorePoint: () => void | Promise<void>; backups: WorkspaceBackupSummary[]; backupLoading: boolean; restorePoint: (backup: WorkspaceBackupSummary) => void }) {
  ...
  return <>
    ...
    <div className="system-status-grid">
     ...
     <section className="system-status-card ok"><span>Application release</span><b>ScopeLogic v1.0 RC5.5.5</b><p>Automatic quote revisions, neutral alternate BOMs, named pricing breakouts, line-level markups, and customer proposal price-display controls are included without changing the cloud schema.</p></section>
...
function QuoteBuilder({ project, quotes, allQuotes, quoteSources, setQuotes, parts, laborRates, difficultyMultipliers, quoteTemplates, saveQuoteTemplate, requestInput, scopeOfWork, message }: { project: Project; quotes: Quote[]; allQuotes: Quote[]; quoteSources:{projectId:string;projectName:string;quote:Quote}[]; setQuotes: (quotes: Quote[]) => void; parts: PartRecord[]; laborRates: LaborRate[]; difficultyMultipliers: DifficultyMultiplier[]; quoteTemplates: QuoteTemplate[]; saveQuoteTemplate:(template:QuoteTemplate)=>void; requestInput:(title:string,body:string,initialValue:string,onConfirm:(value:string)=>void|Promise<void>,confirmLabel?:string)=>void; scopeOfWork:ScopeOfWorkDoc; message:(title:string,body:string)=>void }) {
  ...
  const deleteBreakout=(id:string)=>{if(!quote||!window.confirm('Delete this breakout? Its BOM rows will become Unassigned.'))return;patchQuote({breakouts:(quote.breakouts||[]).filter((breakout)=>breakout.id!==id),lines:quote.lines.map((line)=>line.breakoutId===id?{...line,breakoutId:''}:line)});};
  ...
  const breakoutSummaries=useMemo(()=>{if(!quote||!calc||!(quote.breakouts||[]).length)return[];const baseLines=quote.lines.filter((line)=>!line.alternateId);const summarize=(id:string,name:string,description:string,showOnProposal=true)=>{const lines=baseLines.filter((line)=>(line.breakoutId||'')===id);const material=lines.reduce((sum,line)=>sum+line.qty*line.unitCost*quoteMaterialMarkup(quote,line),0);const labor=fieldLaborRates.reduce((sum,rate)=>{if(rate.id==='engineering'&&quote.engineeringNotRequired)return sum;const minutes=lines.reduce((lineSum,line)=>lineSum+line.qty*legacyLaborMinutes(line,rate.id),0);return sum+(minutes/60)*difficultyMultiplier*rate.costPerHour*laborMarkup(rate);},0);return{id,name,description,showOnProposal,material,labor,other:0,total:material+labor,lineCount:lines.length};};const rows=[...(quote.breakouts||[]).sort((a,b)=>(a.order??0)-(b.order??0)||alphaNumericCompare(a.name,b.name)).map((breakout)=>summarize(breakout.id,breakout.name,breakout.description||'',breakout.showOnProposal!==false))];const unassigned=summarize('','Unassigned BOM','Base BOM rows not assigned to a named breakout',false);if(unassigned.lineCount)rows.push(unassigned);const material=calc.shippingSell-num(quote.jobMaterialDiscount);const labor=calc.pmSell+calc.travelSell+calc.adjustmentSell;const other=calc.otherCostsSell+calc.travelExpense+calc.tax+calc.bond;rows.push({id:'quote-level',name:'General Conditions / Quote-Level Costs',description:'Discounts, taxable and non-taxable costs, travel expenses, tax, bond, and unassigned project labor',showOnProposal:true,material,labor,other,total:material+labor+other,lineCount:0});return rows;},[quote,calc,laborRates,difficultyMultiplier]);
  const generateQuotePdf=async(mode:QuotePdfMode,selectedBomIds?:string[])=>{if(!quote||!calc)return;if(dirty)return message('Save Required','Save the quote before generating a customer PDF so the proposal matches the recorded revision.');if(!['Approved','Awarded'].includes(quote.status))return message('Approval Required','Set the quote status to Approved or Awarded and save it before generating a customer PDF.');if(mode==='full-bom'&&!selectedBomIds?.length)return message('BOM Selection Required','Select at least one purchasing BOM item or choose the No BOM proposal option.');setQuotePdfLoading(true);try{const selectedSet=new Set(selectedBomIds||[]);const pdfLines=mode==='full-bom'?purchasingLines.filter((line)=>selectedSet.has(line.id)&&line.qty>0):[];const bytes=await buildQuotePdfBytes({ mode, pricingDisplay:quotePdfPricingDisplay, project:{name:project.name,client:project.client,versionDate:project.versionDate,revision:project.revision}, quote:{number:quote.number,name:quote.name,groups:quote.groups||[],lines:pdfLines.map((line)=>({groupId:line.groupId||'',description:line.description,qty:line.qty}))}, scope:{includedHtml:combinedScopeOfWorkHtml(scopeOfWork),excludedHtml:''}, totals:{material:calc.materialSell+calc.shippingSell,labor:calc.laborSell,other:calc.otherCostsSell+calc.travelExpense,tax:calc.tax,bond:calc.bond,total:calc.total}, breakouts:breakoutSummaries.filter((row)=>row.showOnProposal).map(({id,name,description,material,labor,other,total})=>({id,name,description,material,labor,other,total})), alternates:alternateSummaries.map(({id,name,scopeHtml,awarded,classification,material,labor,total})=>({id,name,scopeHtml:scopeHtml||'',awarded:Boolean(awarded),classification,material,labor,total})) });const blob=pdfBytesToBlob(bytes);const url=URL.createObjectURL(blob);const anchor=document.createElement('a');anchor.href=url;anchor.download=`${project.name}_${quote.number}_Quote.pdf`.replace(/[^a-z0-9._-]+/gi,'_');document.body.appendChild(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(url),2000);setQuotePdfOpen(false);setBomPdfSelectOpen(false);message('PDF Generated',mode==='full-bom'?'Customer quote PDF with the selected reconciled BOM items was generated.':'Customer quote PDF without BOM was generated.');}catch(error){message('Quote PDF Failed',error instanceof Error?error.message:'The quote PDF could not be generated.');}finally{setQuotePdfLoading(false);}};
  ...
  const matches=allMatches.slice(0,150);
  return <><PageHead eyebrow="Estimating" title="Quote Builder" description={`Build material and labor pricing for ${project.name}. Changes remain local to the quote until Save Quote is clicked.`} action={<div className="button-row"><button className="danger-button" disabled={!quote||quote.locked} onClick={deleteQuote}>Delete Quote</button><button className="primary" disabled={!quote||!dirty} onClick={saveQuote}>{dirty?'Save Quote':'Saved'}</button></div>} />
    <div className="quote-switcher-bar"><label><span>Current Quote</span><select value={selectedId} onChange={(e)=>{setSelectedId(e.target.value);setActiveBomTab('base');setSelectedLineIds([])}}>{sortedQuotes.map((q)=><option key={q.id} value={q.id}>{q.number} — {q.name}</option>)}</select></label><span className="quote-switcher-status">{quote?`${quote.status}${quote.locked?' · Locked':''}${dirty?' · Unsaved':''}`:'No quote selected'}</span><div className="quote-number-actions"><button className="secondary" disabled={!quote||quote.locked} onClick={createRevision}>New Revision</button><button className="secondary" disabled={!quote||quote.status!=='Awarded'} onClick={createChangeOrder}>New Change Order</button><button className="secondary" disabled={!quote} onClick={duplicateQuote}>Duplicate Quote</button><button className="secondary" disabled={!quote} onClick={saveCurrentAsTemplate}>Save as Template</button><button className="secondary" disabled={!quoteSources.length} onClick={()=>{setCopyQuoteSearch('');setCopyQuoteOpen(true)}}>Copy Existing Quote...</button><button className="primary" onClick={addQuote}>+ New Quote</button></div></div><div className="quote-layout quote-layout-full"><div className="quote-workspace">{!quote?<div className="empty-panel"><b>No quotes yet.</b><p>Create the first quote for this project, duplicate a current quote, or copy one from another project.</p></div>:<>
      <section className={`quote-panel quote-top-controls ${quote.locked?'locked-quote':''}`}><div className="quote-panel-head"><div><span>{quote.number}</span><h2>Quote Setup</h2></div><div className="button-row"><button className="secondary" disabled={quote.locked} onClick={()=>setOrganizerOpen(true)}>Group / Reorder</button><button className="secondary" disabled={quote.locked} onClick={()=>{setActiveBomTab('base');setPickerOpen(true)}}>Add Part</button><button className="primary" onClick={()=>['Approved','Awarded'].includes(quote.status)?setQuotePdfOpen(true):message('Approval Required','Set the quote status to Approved or Awarded and save it before generating a customer PDF.')}>Generate Quote PDF</button></div></div><div className="quote-head-fields"><label>Quote #<input value={quote.number} readOnly aria-readonly="true"/><small>Assigned automatically</small></label><label>Quote Name<input disabled={quote.locked} value={quote.name} onChange={(e)=>patchQuote({name:e.target.value})}/></label><label>Status<select disabled={quote.locked} value={quote.status} onChange={(e)=>patchQuote({status:e.target.value})}>{alphaSorted(['Draft','Review','Approved','Awarded','Superseded']).map((status)=><option key={status}>{status}</option>)}</select></label></div><div className="quote-difficulty-global"><div><span className="control-label">Difficulty Adder</span><div className="difficulty-choice-grid"><label><input disabled={quote.locked} type="radio" name="difficulty" checked={!quote.difficultyId} onChange={()=>patchQuote({difficultyId:''})}/> Standard <small>1.00</small></label>{difficultyMultipliers.filter((d)=>d.active).sort((a,b)=>alphaNumericCompare(a.name,b.name)).map((d)=><label key={d.id}><input disabled={quote.locked} type="radio" name="difficulty" checked={quote.difficultyId===d.id} onChange={()=>patchQuote({difficultyId:d.id})}/>{d.name}<small>{d.multiplier.toFixed(2)}</small></label>)}</div></div><label className="global-markup">Global Material Markup<input disabled={quote.locked} type="number" min="0" step="0.01" value={quote.globalMaterialMarkup??1.20} onChange={(e)=>patchQuote({globalMaterialMarkup:num(e.target.value)})}/></label><label className="engineering-toggle"><input disabled={quote.locked} type="checkbox" checked={Boolean(quote.engineeringNotRequired)} onChange={(e)=>patchQuote({engineeringNotRequired:e.target.checked})}/> Engineering Not Required</label></div>{quote.locked&&<div className="quote-lock-banner">This predecessor is locked. Create a new revision to make pricing changes.</div>}</section>
      <section className="quote-panel quote-bom-workspace"><div className="quote-panel-head"><div><span>Base pricing stays separate until an alternate is awarded</span><h2>Base Bid, Alternates & Purchasing BOM</h2></div><div className="button-row"><button className="secondary" disabled={quote.locked} onClick={()=>setBreakoutManagerOpen((value)=>!value)}>Manage Breakouts</button><button className="primary" disabled={quote.locked} onClick={addAlternate}>+ New Alternate</button></div></div>
        <div className="quote-bom-tabs"><button className={activeBomTab==='base'?'active':''} onClick={()=>{setActiveBomTab('base');setSelectedLineIds([])}}>Base Bid</button>{(quote.alternates||[]).sort((a,b)=>alphaNumericCompare(a.name,b.name)).map((item)=>{const summary=alternateSummaries.find((row)=>row.id===item.id);return <button key={item.id} className={`${activeBomTab===item.id?'active':''} ${String(summary?.classification||'').toLowerCase().replace(' ','-')}`} onClick={()=>{setActiveBomTab(item.id);setSelectedLineIds([])}}>{item.name}<small>{summary?.classification||'NO COST'} {money(summary?.total||0)}</small></button>})}<button className={activeBomTab==='purchasing'?'active purchasing':''} onClick={()=>{setActiveBomTab('purchasing');setSelectedLineIds([])}}>Purchasing BOM<small>Base + awarded alternates</small></button></div>
        {activeBomTab==='base'&&breakoutManagerOpen&&<div className="breakout-manager"><div className="breakout-manager-head"><div><span>Quote-specific setup</span><h3>Named Pricing Breakouts</h3></div><button className="secondary" onClick={addBreakout}>+ Add Breakout</button></div>{(quote.breakouts||[]).length?<div className="breakout-editor-list">{[...(quote.breakouts||[])].sort((a,b)=>(a.order??0)-(b.order??0)).map((breakout,index)=><div className="breakout-editor-row" key={breakout.id}><div className="breakout-order"><button disabled={index===0} onClick={()=>moveBreakout(breakout.id,-1)}>↑</button><button disabled={index===(quote.breakouts||[]).length-1} onClick={()=>moveBreakout(breakout.id,1)}>↓</button></div><input aria-label="Breakout name" value={breakout.name} onChange={(e)=>patchBreakout(breakout.id,{name:e.target.value})}/><input aria-label="Breakout description" placeholder="Optional description" value={breakout.description||''} onChange={(e)=>patchBreakout(breakout.id,{description:e.target.value})}/><label><input type="checkbox" checked={breakout.showOnProposal!==false} onChange={(e)=>patchBreakout(breakout.id,{showOnProposal:e.target.checked})}/> Proposal</label><button className="link-button danger" onClick={()=>deleteBreakout(breakout.id)}>Delete</button></div>)}</div>:<div className="empty-panel compact"><b>No named breakouts yet.</b><p>Add any customer-facing accounting section you need, then assign Base Bid BOM rows below.</p></div>}</div>}
        {alternate&&<div className={`alternate-detail ${String(alternateSummaries.find((item)=>item.id===alternate.id)?.classification||'').toLowerCase().replace(' ','-')}`}><div className="alternate-detail-head"><label><span>Alternate Name</span><input disabled={quote.locked} value={alternate.name} onChange={(e)=>patchAlternate(alternate.id,{name:e.target.value})}/></label><label className="award-toggle"><input disabled={quote.locked} type="checkbox" checked={Boolean(alternate.awarded)} onChange={(e)=>patchAlternate(alternate.id,{awarded:e.target.checked})}/><span>Awarded — include in Purchasing BOM</span></label><button className="danger-button" disabled={quote.locked} onClick={()=>deleteAlternate(alternate.id)}>Delete Alternate</button></div><div className="alternate-scope"><span className="control-label">Short Alternate Scope of Work</span>{quote.locked?<div className="locked-rich-text" dangerouslySetInnerHTML={{__html:alternate.scopeHtml||'<p>No alternate scope entered.</p>'}}/>:<RichTextEditor value={alternate.scopeHtml||''} onChange={(scopeHtml)=>patchAlternate(alternate.id,{scopeHtml})} placeholder="Describe what this alternate changes from the specified Base Bid..." />}</div>{(()=>{const summary=alternateSummaries.find((item)=>item.id===alternate.id);return <div className="alternate-totals"><span>Material <b>{money(summary?.material||0)}</b></span><span>Labor <b>{money(summary?.labor||0)}</b></span><span>Alternate Total <b>{money(summary?.total||0)}</b></span><strong>{summary?.classification||'NO COST'}</strong><small>Positive rows add scope; negative rows remove scope.</small></div>})()}</div>}
        {activeBomTab==='purchasing'?<div className="purchasing-bom"><div className="purchasing-note">Read-only final purchasing quantities: Base Bid plus every awarded alternate. Unawarded alternates have no effect.</div><div className="quote-table-wrap"><table className="quote-table purchasing-table"><thead><tr><th>Manufacturer</th><th>Part #</th><th>Description</th><th>Final Qty</th><th>Status</th></tr></thead><tbody>{purchasingLines.map((line)=><tr key={line.id} className={line.qty<0?'purchasing-warning':''}><td>{line.manufacturer}</td><td><b>{line.partNumber}</b></td><td>{line.description}</td><td><b>{line.qty}</b></td><td>{line.qty<0?'Review negative final quantity':'Ready'}</td></tr>)}</tbody></table>{!purchasingLines.length&&<div className="empty-panel compact"><b>No purchasing quantities yet.</b><p>Add Base Bid items or award an alternate.</p></div>}</div></div>:<><div className="quote-items-head"><div><span className="control-label">{activeBomTab==='base'?'Base Bid BOM':`${alternate?.name||'Alternate'} BOM`}</span><small>{activeBomTab==='base'?'Assign each row to an optional named pricing breakout.':'Use positive quantities for additions and negative quantities for removals.'}</small></div><div className="button-row"><button className="secondary" disabled={quote.locked||activeBomTab!=='base'} onClick={()=>setOrganizerOpen(true)}>Group / Reorder</button><button className="secondary" disabled={quote.locked} onClick={()=>setPickerOpen(true)}>+ Add Part</button></div></div><div className="bom-bulk-actions"><label><input type="checkbox" checked={allLinesSelected} onChange={toggleAllQuoteLines}/><span>Select All</span></label><span>{selectedLineIds.length} selected</span><button className="danger-button" disabled={quote.locked||!selectedLineIds.length} onClick={deleteSelectedLines}>Delete Selected</button></div><div className="quote-table-wrap"><table className="quote-table quote-items-table expanded-markup"><thead><tr><th>Select</th><th>BOM</th><th>Breakout</th><th>Manufacturer</th><th>Part #</th><th>Description</th><th>Qty</th><th>Cost</th><th>Ext. Cost</th><th>Markup</th><th>Material Sell</th>{fieldLaborRates.map((rate)=><th key={rate.id}>{rate.name} Min</th>)}<th></th></tr></thead><tbody>{visibleQuoteSectionGroups.flatMap((group)=>[<tr className="bom-section-row" key={`section-${group.id||'ungrouped'}`}><td colSpan={12+fieldLaborRates.length}><b>{group.section}</b></td></tr>,...group.lines.map((line)=><tr key={line.id} className={selectedLineIds.includes(line.id)?'selected-bom-line':''}><td><input type="checkbox" checked={selectedLineIds.includes(line.id)} onChange={()=>toggleLineSelection(line.id)}/></td><td><select className="alternate-assignment" disabled={quote.locked} value={line.alternateId||''} onChange={(e)=>patchLine(line.id,{alternateId:e.target.value,breakoutId:e.target.value?'':line.breakoutId})}><option value="">Base Bid</option>{(quote.alternates||[]).sort((a,b)=>alphaNumericCompare(a.name,b.name)).map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></td><td>{!line.alternateId?<select className="breakout-assignment" disabled={quote.locked} value={line.breakoutId||''} onChange={(e)=>patchLine(line.id,{breakoutId:e.target.value})}><option value="">Unassigned</option>{[...(quote.breakouts||[])].sort((a,b)=>(a.order??0)-(b.order??0)).map((breakout)=><option key={breakout.id} value={breakout.id}>{breakout.name}</option>)}</select>:<span className="muted-cell">Alternate</span>}</td><td>{line.manufacturer}{line.adHoc&&<small className="adhoc-tag">AD-HOC</small>}</td><td><b>{line.partNumber}</b></td><td><input disabled={quote.locked} className="description-input" value={line.description} onChange={(e)=>patchLine(line.id,{description:e.target.value})}/></td><td><input disabled={quote.locked} className="qty-input" type="number" min={line.alternateId?undefined:0} value={line.qty} onChange={(e)=>patchLineQty(line,num(e.target.value))}/></td><td><input disabled={quote.locked} className="money-input" type="number" step="0.01" value={line.unitCost} onChange={(e)=>patchLine(line.id,{unitCost:num(e.target.value)})}/></td><td>{money(line.qty*line.unitCost)}</td><td><div className="line-markup"><input disabled={quote.locked} type="number" min="0" step="0.01" value={quoteMaterialMarkup(quote,line)} onChange={(e)=>patchLine(line.id,{materialMarkupOverride:num(e.target.value)})}/>{Number.isFinite(line.materialMarkupOverride)?<button disabled={quote.locked} onClick={()=>patchLine(line.id,{materialMarkupOverride:null})}>Use global</button>:<small>Global</small>}</div></td><td><b>{money(line.qty*line.unitCost*quoteMaterialMarkup(quote,line))}</b></td>{fieldLaborRates.map((rate)=><td key={rate.id}><input disabled={quote.locked} className="labor-min-input" type="number" min="0" value={legacyLaborMinutes(line,rate.id)} onChange={(e)=>patchLineLabor(line,rate.id,num(e.target.value))}/></td>)}<td><button className="link-button danger" disabled={quote.locked} onClick={()=>{patchQuote({lines:quote.lines.filter((item)=>item.id!==line.id)});setSelectedLineIds((ids)=>ids.filter((id)=>id!==line.id));}}>Remove</button></td></tr>)])}</tbody></table></div></>}
      </section>
      {calc&&breakoutSummaries.length>0&&<section className="quote-panel breakout-summary-panel"><div className="quote-panel-head"><div><span>Named accounting view</span><h2>Breakout Pricing Summary</h2></div></div><div className="quote-table-wrap"><table className="breakout-summary-table"><thead><tr><th>Breakout Price</th><th>Material</th><th>Labor</th><th>Other / Fees</th><th>Total Price</th></tr></thead><tbody>{breakoutSummaries.map((row)=><tr key={row.id}><td><b>{row.name}</b><small>{row.description}</small></td><td>{money(row.material)}</td><td>{money(row.labor)}</td><td>{money(row.other)}</td><td><b>{money(row.total)}</b></td></tr>)}</tbody><tfoot><tr><td>BASE BID TOTAL</td><td>{money(breakoutSummaries.reduce((sum,row)=>sum+row.material,0))}</td><td>{money(breakoutSummaries.reduce((sum,row)=>sum+row.labor,0))}</td><td>{money(breakoutSummaries.reduce((sum,row)=>sum+row.other,0))}</td><td>{money(calc.total)}</td></tr></tfoot></table></div></section>}
      {calc&&<section className="quote-panel pricing-summary"><div className="quote-panel-head"><div><span>Pricing summary</span><h2>Labor, Travel & Quote Totals</h2></div></div><div className="pricing-summary-layout"><div className="pricing-notes-column"><div className="travel-calculator"><h3>Travel Time Calculator</h3><label><span>Crew Size</span><input type="number" min="0" value={quote.travel?.crewSize??1} onChange={(e)=>patchQuote({travel:{...(quote.travel||blankQuote('',1).travel!),crewSize:num(e.target.value)}})}/></label><label><span>Round Trip Travel Time (hrs)</span><input type="number" min="0" step="0.25" value={quote.travel?.roundTripHours??0} onChange={(e)=>patchQuote({travel:{...(quote.travel||blankQuote('',1).travel!),roundTripHours:num(e.target.value)}})}/></label><label><span>Total Days</span><input type="number" min="0" value={quote.travel?.days??1} onChange={(e)=>patchQuote({travel:{...(quote.travel||blankQuote('',1).travel!),days:num(e.target.value)}})}/></label><label><span>Travel Labor Type</span><select value={quote.travel?.laborRateId||'installation'} onChange={(e)=>patchQuote({travel:{...(quote.travel||blankQuote('',1).travel!),laborRateId:e.target.value}})}>{laborRates.filter((r)=>r.active).sort((a,b)=>alphaNumericCompare(a.name,b.name)).map((r)=><option key={r.id} value={r.id}>{r.name}</option>)}</select></label><div className="travel-result"><span>Travel Labor Hours</span><b>{calc.travelHours.toFixed(2)}</b></div></div><div className="travel-calculator"><h3>Hotel & Per Diem Calculator</h3><label><span>Total Hotel Nights</span><input type="number" min="0" value={quote.travel?.hotelNights??0} onChange={(e)=>patchQuote({travel:{...(quote.travel||blankQuote('',1).travel!),hotelNights:num(e.target.value)}})}/></label><label><span>Room Rate / Night</span><input type="number" min="0" step="0.01" value={quote.travel?.roomRate??0} onChange={(e)=>patchQuote({travel:{...(quote.travel||blankQuote('',1).travel!),roomRate:num(e.target.value)}})}/></label><label><span>Per Diem Rate / Man / Day</span><input type="number" min="0" step="0.01" value={quote.travel?.perDiemRate??0} onChange={(e)=>patchQuote({travel:{...(quote.travel||blankQuote('',1).travel!),perDiemRate:num(e.target.value)}})}/></label><div className="travel-result"><span>Hotel (1 room / man)</span><b>{money(calc.hotelCost)}</b></div><div className="travel-result"><span>Per Diem</span><b>{money(calc.perDiemCost)}</b></div><div className="travel-result total"><span>Hotel + Per Diem</span><b>{money(calc.travelExpense)}</b></div></div><label className="summary-field"><span>Terms</span><input value={quote.terms||''} onChange={(e)=>patchQuote({terms:e.target.value})}/></label><div className="summary-notes-fill"><label className="summary-field textarea-field"><span>Internal Notes</span><textarea value={quote.internalNotes||''} onChange={(e)=>patchQuote({internalNotes:e.target.value})}/></label><label className="summary-field textarea-field"><span>Admin Notes</span><textarea value={quote.adminNotes||''} onChange={(e)=>patchQuote({adminNotes:e.target.value})}/></label></div></div>
        <div className="pricing-totals-column"><table className="pricing-summary-table"><thead><tr><th>Materials</th><th>Cost</th><th>Markup</th><th>Price</th></tr></thead><tbody><tr><td>Other Taxable Items Cost (Consumables, Shipping)</td><td><input type="number" step="0.01" value={quote.shipping} onChange={(e)=>patchQuote({shipping:num(e.target.value)})}/></td><td><input aria-label="Other taxable items markup" type="number" min="0" step="0.01" value={quote.shippingMarkup??quote.globalMaterialMarkup??1.20} onChange={(e)=>patchQuote({shippingMarkup:num(e.target.value)})}/></td><td>{money(calc.shippingSell)}</td></tr><tr><td>Job Gross Material Discounts</td><td><input type="number" step="0.01" value={quote.jobMaterialDiscount??0} onChange={(e)=>patchQuote({jobMaterialDiscount:num(e.target.value)})}/></td><td></td><td>-{money(num(quote.jobMaterialDiscount))}</td></tr><tr className="summary-total"><td>MATERIALS TOTAL</td><td>{money(calc.materialCost+quote.shipping)}</td><td></td><td>{money(calc.materialSell+calc.shippingSell)}</td></tr></tbody></table>
          ...
          <table className="pricing-summary-table"><tbody><tr><td>Travel Time Labor ({calc.travelHours.toFixed(2)} hrs)</td><td>{money(calc.travelCost)}</td><td>{calc.travelMarkup.toFixed(2)}</td><td>{money(calc.travelSell)}</td></tr><tr><td>Hotel Cost</td><td>{money(calc.hotelCost)}</td><td></td><td>{money(calc.hotelCost)}</td></tr><tr><td>Per Diem</td><td>{money(calc.perDiemCost)}</td><td></td><td>{money(calc.perDiemCost)}</td></tr><tr><td>Other Non-Taxable Job Costs (permits, lift rental, misc.)</td><td><input type="number" step="0.01" value={quote.otherCosts} onChange={(e)=>patchQuote({otherCosts:num(e.target.value)})}/></td><td><input aria-label="Other non-taxable job costs markup" type="number" min="0" step="0.01" value={quote.otherCostsMarkup??1} onChange={(e)=>patchQuote({otherCostsMarkup:num(e.target.value)})}/></td><td>{money(calc.otherCostsSell)}</td></tr><tr className="summary-total"><td>SUBTOTAL</td><td>{money(calc.directCost)}</td><td></td><td>{money(calc.subtotal)}</td></tr><tr><td>Tax</td><td colSpan={2}><input type="number" step="0.01" value={quote.taxRate} onChange={(e)=>patchQuote({taxRate:num(e.target.value)})}/> %</td><td>{money(calc.tax)}</td></tr><tr><td>Bond Required</td><td colSpan={2}><input type="number" step="0.01" value={quote.bondRate} onChange={(e)=>patchQuote({bondRate:num(e.target.value)})}/> %</td><td>{money(calc.bond)}</td></tr></tbody></table><div className="quote-total-card compact"><div className="grand"><span>QUOTE TOTAL</span><b>{money(calc.total)}</b></div><div><span>Gross Margin</span><b>{calc.grossMargin.toFixed(2)}%</b></div><div><span>Gross Profit</span><b>{money(calc.grossProfit)}</b></div></div></div></div><div className="quote-savebar"><span>{dirty?'Unsaved quote changes':'Quote is saved'}</span><div className="button-row"><button className="danger-button" disabled={quote.locked} onClick={deleteQuote}>Delete Quote</button><button className="primary" disabled={!dirty} onClick={saveQuote}>{dirty?'Save Quote':'Saved'}</button></div></div></section>}
    </>}</div></div>
