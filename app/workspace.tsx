...
const compareCatalogParts = (a: PartRecord, b: PartRecord) => a.manufacturer.localeCompare(b.manufacturer, undefined, { sensitivity: 'base', numeric: true })
  || a.partNumber.localeCompare(b.partNumber, undefined, { sensitivity: 'base', numeric: true })
  || a.description.localeCompare(b.description, undefined, { sensitivity: 'base' });
const SOURCE_TYPE_OPTIONS = ['Drawing', 'Specification', 'Addendum', 'RFI / ASI', 'Existing Condition', 'Owner Direction', 'Scope Omission', 'Minimum System Requirement', 'Other'];
const accessRuleScenarios = ['Custom', 'Single Reader Door', 'Double Reader Door', 'Maglock Door', 'Electric Strike Door', 'Electrified Lockset Door', 'Door Contact Only', 'REX Only', 'Access Control Panel Capacity', 'Power Supply Capacity'];
const cctvRuleScenarios = ['Custom', 'Indoor Dome Camera', 'Outdoor Dome Camera', 'Outdoor Bullet Camera', 'PTZ Camera', 'Camera License', 'Camera Mount / Accessory', 'PoE / Switch Port Capacity', 'NVR / Recorder Channel Capacity', 'Camera Cable Run'];
  ...
  return <>
    ...
    <section className="issue-editor matrix-editor-full">
      {!draft ? <div className="empty-state large"><b>Select a submitted SLR below or create a new issue.</b><p>Only submitted entries feed deliverables and PDFs.</p></div> : <>
        ...
        <div className="matrix-reference-grid"><label className="field"><span>SLR ID</span><input value={draft.id} disabled /></label><label className="field"><span>Source Type</span><select value={draft.sourceType||''} onChange={(event)=>patch('sourceType',event.target.value)}><option value="">Select source type...</option>{SOURCE_TYPE_OPTIONS.map((option)=><option key={option}>{option}</option>)}</select></label><Field label="Source Reference" value={draft.reference} onChange={(value) => patch('reference', value)} /><label className="field"><span>Snippet ID</span><input value={draft.snippet&&draft.snippet!=='pending'?draft.snippet:'Assigned when requested / submitted'} disabled /></label><label className="field"><span>Markup Reference</span><input value={draft.id} disabled /></label></div><p className="help-text reference-help">Use Source Reference for the contract-document citation, such as A601 / Note 4 or Division 28 13 00. The SLR ID is the permanent ScopeLogic cross-reference used on markups and downstream deliverables.</p>
