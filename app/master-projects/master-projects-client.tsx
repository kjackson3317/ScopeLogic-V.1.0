...
export default function MasterProjectsClient({ actualUserId, workspaceOwnerId, role, userName }: Props) {
  ...
  const createMaster = async () => {
    ...
    await load();
    setConfirmation('Master Project created successfully.');
  };
  ...
  const createEngagement = async () => {
    ...
    if (selected.systems.length) {
      ...
      if (systemResult.error) systemsWarning = ` Systems were not copied: ${systemResult.error.message}`;
    }
    ...
    await load();
    setConfirmation(`Client Engagement created successfully.${systemsWarning}`);
  };
  ...
  const linkLegacyProject = async (engagement: Engagement) => {
    ...
    await load();
    setConfirmation('Existing project linked to a new Master Project successfully.');
  };
