...
import { DEMO_MASTER_ID, DEMO_PROJECT_ID, isEmployerDemo } from '../lib/demo/config';
import {
...
type Engagement = { legacyId: string; label: string };
type DialogIntent = 'preview' | 'official';
type Loaded = { masterId: string; data: ReviewReleaseData; engagements: Engagement[] };

const LOCAL_WORKSPACE_KEYS = [
  'scopelogic-r14-8','scopelogic-r14-7','scopelogic-r14-6','scopelogic-r14-5',
  'scopelogic-r14-4','scopelogic-r14-3','scopelogic-r14-2','technology-preconstruction-workspace',
];
const ALL_KINDS: ReviewReleaseKind[] = REVIEW_RELEASE_OPTIONS.map((item) => item.kind);
...
function activeLegacyProjectId() {
  ...
  for (const key of LOCAL_WORKSPACE_KEYS) {
    ...
    try {
      ...
      // Continue through older recovery keys.
    }
  }
  return isEmployerDemo ? DEMO_PROJECT_ID : '';
}

async function resolveMasterId(supabase: any) {
  const pathId = window.location.pathname.match(/^\/master-projects\/([^/]+)/)?.[1] || '';
  if (pathId) return pathId;
  if (isEmployerDemo) return DEMO_MASTER_ID;
...
export default function ReviewReleaseController() {
  ...
  const [selected, setSelected] = useState<ReviewReleaseKind[]>(ALL_KINDS);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!masterId) throw new Error('No active Master Project could be resolved.');

      const projectsQuery = isEmployerDemo
        ? Promise.resolve({ data: [{ legacy_id: DEMO_PROJECT_ID, client: 'Municipal Facilities Department', engagement_type: 'Demo Engagement', engagement_label: 'Municipal Facilities Department' }], error: null })
        : supabase.from('projects').select('legacy_id,client,engagement_type,engagement_label').eq('master_project_id', masterId);
      ...
      const data: ReviewReleaseData = {
        master: master.data,
        findings: (findings.data || []).map((item: any) => ({ ...item, systems: Array.isArray(item.systems) ? item.systems : [] })),
        actions: actions.data || [],
        checklist: checklist.data || [],
        bids: bids.data || [],
      };
      const engagements: Engagement[] = (projects.data || [])
        .filter((item: any) => item.legacy_id)
        .map((item: any) => ({
          legacyId: String(item.legacy_id),
          label: String(item.engagement_label || item.client || item.engagement_type || item.legacy_id),
        }));
      const active = activeLegacyProjectId();
      const preferred = engagements.some((item) => item.legacyId === active)
        ? active
        : engagements.length === 1
          ? engagements[0].legacyId
          : '';
      setLoaded({ masterId, data, engagements });
  ...
  useEffect(() => {
    const clickHandler = (event: MouseEvent) => {
      ...
      if (/^Generate Official (GC )?Release$/i.test((control.textContent || '').trim())) {
        event.preventDefault();
        event.stopImmediatePropagation();
        showDialog('official');
      }
    ...
    const install = () => {
      if (/^\/master-projects\/[^/]+\/deliverables\/?$/.test(window.location.pathname)) {
        ...
        const printButton = topbar ? Array.from(topbar.querySelectorAll<HTMLButtonElement>('button')).find((button) => button.textContent?.trim() === 'Print / Save PDF' || button.textContent?.trim() === 'Download PDF Preview') : undefined;
        if (printButton) {
      ...
      document.querySelectorAll<HTMLElement>('.sl-deliverable-preview .sl-preview-actions').forEach((actionsHost) => {
        if (!actionsHost.querySelector('[data-review-release-injected="preview"]')) {
          ...
          actionsHost.prepend(preview);
        }
        if (!actionsHost.querySelector('[data-review-release-injected="official"]')) {
          const official = document.createElement('button');
          official.type = 'button';
          official.textContent = 'Generate Official Release';
    ...
    const refresh = () => {
      if (queued) return;
      queued = true;
      window.requestAnimationFrame(() => { queued = false; install(); });
    };
  ...
  const toggle = (kind: ReviewReleaseKind) => {
  ...
  const createOfficial = async () => {
    if (!loaded || !selected.length || !selectedEngagement) return;
    setWorking(true);
    setError('');
    setMessage('');
    try {
      ...
      setMessage(`Official Release ${String(archived.releaseNumber || releaseNumber).padStart(3, '0')} was archived and downloaded.`);
    } catch (cause) {
  ...
  if (!open) return null;
  const officialDisabled = working || loading || !loaded || !selected.length || !selectedEngagement;
  ...
  return <div className="review-release-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !working) setOpen(false); }}>
    <section className="review-release-dialog" role="dialog" aria-modal="true" aria-label="Review deliverable PDF release">
      ...
      {loaded && !loading ? <div className="review-release-body">
        <div className="review-release-selection-head"><div><b>Client Deliverables</b><small>Select the documents to combine into this PDF.</small></div><div><button type="button" data-review-release-owned="true" onClick={() => setSelected(ALL_KINDS)}>Select All</button><button type="button" data-review-release-owned="true" onClick={() => setSelected([])}>Clear All</button></div></div>
        <div className="review-release-options">{REVIEW_RELEASE_OPTIONS.map((item) => <label key={item.kind} className={selected.includes(item.kind) ? 'selected' : ''}><input type="checkbox" checked={selected.includes(item.kind)} onChange={() => toggle(item.kind)} /><span><b>{item.label}</b>{item.kind === 'clarification-log' ? <small>Includes GC Clarifications and RFIs.</small> : item.kind === 'bid-report' ? <small>Missing bidder pricing remains Unpriced.</small> : null}</span></label>)}</div>
        {loaded.engagements.length ? <label className="review-release-field"><span>Official Release Archive</span><select value={selectedEngagement} onChange={(event) => setSelectedEngagement(event.target.value)}><option value="">Select client engagement…</option>{loaded.engagements.map((item) => <option key={item.legacyId} value={item.legacyId}>{item.label}</option>)}</select></label> : <div className="review-release-warning"><b>No linked client engagement.</b><span>PDF Preview is still available; an official release requires an archive owner.</span></div>}
        ...
        <div className="review-release-behavior"><div><b>PDF Preview</b><span>Downloads a PDF only. No number, archive, or lock is created.</span></div><div><b>Official Release</b><span>Creates the numbered immutable archive record and downloads the archived PDF.</span></div></div>
      </div> : null}
      <footer><span>{selected.length} of {REVIEW_RELEASE_OPTIONS.length} deliverables selected</span><div><button type="button" data-review-release-owned="true" disabled={working} onClick={() => setOpen(false)}>Cancel</button><button type="button" data-review-release-owned="true" disabled={working || loading || !loaded || !selected.length} onClick={() => void createPreview()}>{working && intent === 'preview' ? 'Generating…' : 'Download PDF Preview'}</button><button type="button" data-review-release-owned="true" className="primary" disabled={officialDisabled} onClick={() => void createOfficial()}>{working && intent === 'official' ? 'Generating…' : 'Generate Official Release'}</button></div></footer>
    </section>
  </div>;
}
